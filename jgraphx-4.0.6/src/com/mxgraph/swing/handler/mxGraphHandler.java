		...
		DropTargetListener
{
			...
			if (graphComponent.isDragEnabled() && first != null) {
				...
				if (th instanceof mxGraphTransferHandler) {
					final mxGraphTransferable t =
							(mxGraphTransferable) ((mxGraphTransferHandler) th).createTransferable(graphComponent);
	...
	public void setPreviewBounds(Rectangle bounds) {
		if (!Objects.equals(bounds, previewBounds)) {
			...
			if (dirty != null) {
				graphComponent.getGraphControl().repaint(dirty.x - 1, dirty.y - 1,
														 dirty.width + 2, dirty.height + 2);
			}
		}
	}
		...
		if (canImport) {
			...
			if (handler != null) {
				...
				if (transferBounds != null && dragImage != null) {
					dx = (int) Math.round(
							(dragImage.getIconWidth() - 2 - transferBounds.getWidth() * scale) / 2);
					dy = (int) Math.round(
							(dragImage.getIconHeight() - 2 - transferBounds.getHeight() * scale) / 2);
					pt.translate(-dx, -dy);
				}
	...
	protected Point getPreviewLocation (MouseEvent e, boolean gridEnabled) {
		...
		if (first != null && cellBounds != null) {
			...
			x = (int) Math.round((dxg + trans.getX()) * scale)
					+ (int) Math.round(bbox.getX()) - (int) Math.round(cellBounds.getX());
			y = (int) Math.round((dyg + trans.getY()) * scale)
					+ (int) Math.round(bbox.getY()) - (int) Math.round(cellBounds.getY());
		}
						...
						if (tmp == null) {
							if (!graphComponent.isToggleEvent(e))
								...
								graphComponent.getCanvas()
										.hitSwimlaneContent(graphComponent, graph.getView().getState(tmp),
															e.getX(), e.getY()))
	...
	protected MouseEvent createEvent (DropTargetEvent e) {
		...
		// DropTargetEvent is the common superclass, but does not specify getLocation, getDropAction
